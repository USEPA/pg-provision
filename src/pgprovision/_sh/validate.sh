...
os_detect() {
  if [[ -r /etc/os-release ]]; then
    ...
    case "${ID:-}" in
      ...
      *) echo "Unsupported OS ID: ${ID:-unknown}" 1>&2; exit 2 ;;
    esac
    OS_VERSION_ID="${VERSION_ID:-}"; OS_CODENAME="${UBUNTU_CODENAME:-}"
  else
    echo "/etc/os-release not found; cannot detect OS" 1>&2; exit 2
  fi
}
...
paths="$(os_get_paths)" || { echo "[validate][error] os_get_paths failed" >&2; exit 2; }
eval "$paths"
